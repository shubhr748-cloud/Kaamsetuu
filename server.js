...
// Never share this key with customers/workers.
const ADMIN_KEY = process.env.ADMIN_KEY;

if (!ADMIN_KEY) {
  console.warn("WARNING: ADMIN_KEY is not configured.");
}
...
// Customers and workers CANNOT use
// these routes without the secret admin key.
...
app.post(
  ...
  (req, res) => {
    ...
    /*
      ...
      Currently the OTP is printed in
      Render logs for testing.

      Later we will connect a real SMS
      provider here.

      NEVER show the OTP to customers
      in production.
    */

    console.log(
      `KaamSetu OTP for ${phone}: ${otp}`
    );
...
app.post(
  ...
  (req, res) => {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Worker and category are required"
      });
...
app.post(
  ...
  (req, res) => {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Valid booking and rating required"
      });
...
// IMPORTANT:
// These APIs are NOT shown in the
// customer or worker interface.
//
// Access requires:
